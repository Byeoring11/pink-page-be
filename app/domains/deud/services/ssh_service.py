...
class DeudSSHService:
    ...
    async def execute_shell_controller(self, websocket: WebSocket, server_type: int, cusno_list: list, event: asyncio.Event):
    ...
    async def _execute_wdexgm1p(self, websocket: WebSocket, server_type: int, cusno_list: list):
        try:
            ...
            _ = await shell.read_output()

            await shell.send_command('2')
            _ = await shell._save_initial_prompt()

            command = f"sh vmyp_postgresql_dat_ddts.sh {cusno_list.join(',')}"
    ...
    async def _execute_edwap1t(self, websocket: WebSocket, cusno_list: list):
        pass

    async def _execute_mypap1d(self, websocket: WebSocket, cusno_list: list):
        pass
